  ...
  async function onSubmit() {
    ...
    try {
      ...
      const data = (await res.json()) as
        | {
          ok: true;
          user: { role: "PROVIDER_ADMIN" | "STORE_OWNER" | "MANAGER" | "CASHIER" };
        }
      ...
      if (data.user.role !== "PROVIDER_ADMIN") {
        await fetch("/api/auth/logout", { method: "POST" });
        throw new Error("This login is for the SaaS provider only.");
      }
    ...
    <div className="min-h-screen bg-zinc-50">
      <div className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-4">
        <div className="rounded-2xl border border-zinc-200 bg-white p-6">
          <div className="text-xl font-semibold tracking-tight">Provider login</div>
          <p className="mt-1 text-sm text-zinc-600">Sign in as the SaaS provider admin.</p>
          ...
          <div className="mt-6 text-sm text-zinc-600">
            Need a provider admin account?{" "}
            <Link href="/setup" className="font-medium text-zinc-950">
              Run setup
            </Link>
            .
          </div>
